        ...
        <section className="bg-white rounded-3xl shadow-sm p-6">
          <div className="flex justify-between items-center mb-6">
            ...
            <div className="space-y-1">
              {studentBooked.map((reservation, index) => (
            ...
            <Carousel className="w-full">
              <CarouselContent>
                {mentors.map((ment) => (
                  ...
                  >
                    <div
                      className="bg-slate-100 rounded-xl p-6 text-center font-semibold cursor-pointer"
                      onClick={() => push(`/mentor/${ment.id}`)}
                    >
                      {ment.firstname}
                    </div>
                    <div className="mt-3">
                      <img
                        src={ment.profilePic ?? FALLBACK_AVATAR}
                        alt={`${ment.firstname} profile`}
                      />
                    </div>
                  </CarouselItem>
                ))}
          ...
          <Carousel className="mt-8">
            <CarouselContent>
              {mentors.map((ment) => (
                ...
                >
                  <div
                    onClick={() => push(`/student/mentorProfile/${ment.id}`)}
                  ...
                  >
                    ...
                    <button
                      type="button"
                      className="mt-4 w-full bg-indigo-600 text-white py-2 rounded-xl text-sm font-medium hover:bg-indigo-700 transition"
                    >
                      View Profile
                    </button>
                  </div>

...
const Page = () => {
  ...
  const handleBooking = useCallback(async (id: AvailableDate["id"]) => {
    if (!studentId) return;
   const res=  await fetch("/api/mentorAvailability/booking", {
    ...
    if (res.ok) {
      ...
      // Optionally refresh the dates or close the dialog
      await fetchDates();
    }
  }, [studentId, fetchDates]);

  return (
    <div className="min-h-screen bg-[#020202] flex text-slate-200 selection:bg-indigo-500/30">
      <SideBar
      ...
      <div className="flex-1 flex flex-col h-screen overflow-y-auto no-scrollbar relative">
        ...
        <header className="px-8 py-6 sticky top-0 z-20 bg-[#020202]/50 backdrop-blur-xl border-b border-white/5">
          <div className="flex items-center text-[10px] font-black uppercase tracking-[0.3em] text-slate-500">
            <LayoutGrid size={12} className="mr-2" />
            <span className="cursor-pointer hover:text-indigo-400 transition-colors">Workspace</span>
            <ChevronRight size={10} className="mx-3 opacity-30" />
            <span className="text-indigo-500">Scheduling Engine</span>
          </div>
        </header>

        <main className="p-6 md:p-12 max-w-[1500px] mx-auto w-full space-y-12">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
            ...
            <div className="lg:col-span-7 space-y-10">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 gap-3 max-h-[420px] overflow-y-auto pr-2 no-scrollbar">
                    {availableDate.filter(s => s.status === "AVAILABLE").length > 0 ? (
                        ...
                        <Dialog key={slot.id}>
                          <DialogTrigger asChild>
                            <motion.button 
                              ...
                              className="group flex items-center justify-between p-6 bg-[#0a0a0a] border border-white/5 rounded-3xl hover:border-indigo-500/50 transition-all text-left"
                            >
                              <div className="flex items-center gap-4">
                                <div className="p-3 bg-white/5 rounded-2xl group-hover:bg-indigo-500 group-hover:text-white transition-all">
                                  <Clock size={20} />
                                </div>
                                <div>
                          ...
                          <DialogContent className="bg-[#0a0a0a] border-white/10 text-white rounded-[2.5rem] p-8">
                            ...
                            <div className="mt-6 p-8 bg-indigo-600 rounded-3xl text-center">
                               <p className="text-[10px] font-black uppercase tracking-[0.2em] text-white/60 mb-2">Live Window</p>
                               <p className="text-2xl font-black tracking-tighter">{slot.startTime} — {slot.endTime}</p>
                            </div>
                            <button 
                              className="mt-6 w-full py-4 bg-white text-black rounded-2xl font-black uppercase text-xs tracking-[0.2em] hover:bg-indigo-500 hover:text-white transition-all"
                      ...
                      <div className="py-20 text-center bg-white/[0.02] rounded-3xl border border-dashed border-white/10">
                        <p className="text-[10px] text-slate-600 font-black uppercase tracking-widest">Registry Empty for this date</p>
                      </div>
                    )}
                  </div>
                </div>
            ...
            <div className="lg:col-span-5">
              <div className="sticky top-28 space-y-6">
                ...
                <div className="relative overflow-hidden rounded-[3rem] bg-[#0a0a0a] border border-white/10 p-10 group shadow-2xl">
                  <div className="absolute top-0 right-0 w-64 h-64 bg-indigo-600/10 rounded-full -mr-32 -mt-32 blur-[100px] group-hover:bg-indigo-600/20 transition-all duration-700" />
                  ...
                  <div className="relative z-10 space-y-8">
                    ...
                    <h1 className="text-4xl md:text-5xl font-black leading-none tracking-tighter text-white uppercase italic">
                      {course?.courseTitle || "Encrypting..."}
                    </h1>
                    ...
                    <div className="pt-8 border-t border-white/5 flex items-end justify-between">
                      <div>
                        ...
                        <p className="text-4xl font-black text-white tracking-tighter">{course?.paymentValue ?? "—"}</p>
                      </div>
                ...
                {/* Technical Specs */}
                <div className="bg-[#0a0a0a] rounded-[2.5rem] p-10 border border-white/5 space-y-8">
                  ...
                  <div className="text-slate-400 text-sm leading-relaxed font-medium">
                    {course?.courseInfo || "Loading protocol documentation..."}
                  </div>

                  <div className="space-y-3">
                    {["60min Strategic Deep-Dive", "Full Neural Resource Access", "Verified Certificate"].map((item) => (
                      <div key={item} className="flex items-center gap-4 text-[10px] font-black text-slate-400 bg-white/[0.02] p-4 rounded-2xl border border-white/5 uppercase tracking-widest">
                  ...
                  <div className="pt-6 flex items-center justify-between border-t border-white/5 opacity-50 grayscale hover:grayscale-0 transition-all">
